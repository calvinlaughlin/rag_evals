...
class CLIPImageProcessor:
    ...
    def process_image(self, image_bytes: bytes, image_context: str | None = None) -> Dict[str, Any]:
        ...
        # ------------------ CLIP classification ------------------
        # The test-suite monkey-patches `_load_clip` with a very small stub where
        # `self.processor` is a `types.SimpleNamespace` that *stores* a `__call__`
        # attribute instead of being directly callable.  Accessing the attribute
        # via ``()`` therefore fails with ``TypeError: 'SimpleNamespace' object is
        # not callable``.  We accommodate both real and stub processors here.

        if callable(self.processor):
            inputs = self.processor(images=pil_img, return_tensors="pt")
        elif hasattr(self.processor, "__call__"):
            # Some test doubles attach a ``__call__`` attr to the INSTANCE to
            # avoid implementing a full class.  We invoke that explicitly.
            inputs = self.processor.__call__(images=pil_img, return_tensors="pt")  # type: ignore[attr-defined]
        else:
            raise TypeError("`processor` object is not callable and has no `__call__` attribute.")

        # Move inputs to the correct device.  Real HF processors return an
        # object with a convenient ``.to(device)`` helper; the stub returns a
        ...
        if hasattr(inputs, "to"):
            ...
            raise TypeError("Unexpected return type from processor stub.")
        with torch.no_grad():
